...
def get_self_critical_reward(greedy_res, data_gts, gen_result, opt):
    ...
    seq_per_img = gen_result_size // len(
        data_gts
    )  # gen_result_size  = batch_size * seq_per_img
    ...
    scores = (
        scores[:gen_result_size].reshape(batch_size, seq_per_img)
...
def get_self_cider_scores(data_gts, gen_result, opt):
    ...
    for i in range(len(data_gts)):
        ...
        def get_div(eigvals):
            ...
            return -np.log(np.sqrt(eigvals[-1]) / (np.sqrt(eigvals).sum())) / np.log(
                len(eigvals)
            )
